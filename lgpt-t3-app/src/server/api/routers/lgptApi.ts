...
export const lgptApiRouter = createTRPCRouter({
        ...
        .mutation<string>(async ({ input }) => {
            ...
            try {
                const response = await fetch(
                    `${env.GPT_URL}${env.GPT_SAVE_ROUTE}`,
                    {
                        method: "POST",
                        body: formData,
                    }
    ...
    postPrompt: publicProcedure
        ...
        .mutation<PromptResponse>(async ({ input }) => {
            ...
            try {
                const response = await fetch(
                    `${env.GPT_URL}${env.GPT_PROMPT_ROUTE}?${
                        env.GPT_PROMPT_BODY_KEY
                ...
                if (!response.ok) {
                    throw new Error(
                        `Failed to call lgpt API: ${response.statusText}`
                    );
                }
    ...
    runIngest: publicProcedure.mutation<boolean>(async () => {
        try {
            const response = await fetch(
                `${env.GPT_URL}${env.GPT_INGEST_ROUTE}`,
                {
                    method: "GET",
                    keepalive: true,
    ...
    deleteSource: publicProcedure.mutation<boolean>(async () => {
        try {
            const response = await fetch(
                `${env.GPT_URL}${env.GPT_DELETE_ROUTE}`,
                {
                    method: "GET",
                }
            );
            ...
            if (!isDeleteResponse(rawResponse)) {
                ...
                if (deleteResponse.message.includes(`successfully deleted`)) {
                    ...
                    throw new Error(
                        `Failed to delete and recreate source directory: ${response.statusText}`
                    );
                }
            }
        } catch (error) {
