...
const GptDropzone: React.FC = () => {
    ...
    const onDrop = useCallback((acceptedFiles: File[]) => {
        ...
        acceptedFiles.forEach((file) => {
            fileToBase64(file)
                .then(fileData => {
                    saveDocumentMutation.mutateAsync(fileData)
                        .then(isUploaded => {
                            if (isUploaded) {
    ...
    const rejectionFeedback = fileRejections.flatMap(rejection => rejection.errors.map(error => error.message));
            ...
            {!isUploading && !isIngesting &&
                <div className="mt-4">
                    <div {...getRootProps()} style={{ border: '2px dashed gray', padding: '20px', textAlign: 'center' }}>
                        <input {...getInputProps()} />
                            {isDragReject ?
                                    <p>Unsupported file type. Please drop only PDF, TXT, CSV, XLS, or XLSX files.</p> :
                                    isDragActive ?
                                        <p>Drop the files here ...</p> :
                                <p>
                                    Drag & drop files here to upload and ingest, or click to select files<br /> 
                                    Supported file types: PDF, TXT, CSV, XLS, or XLSX (100 Mb max size)
                                </p>
                            }
                    </div>
